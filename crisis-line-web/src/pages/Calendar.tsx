...
import { getAllEvents, getSignUpsForEvent, signUpUserToEvent, cancelSignUpForEvent, getAllEventSignUps } from '../services/eventService';
...
const Calendar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [allEvents, allSignUps, users] = await Promise.all([
        getAllEvents(),
        getAllEventSignUps(),
        getAllUsers()
      ]);
  ...
  const getEventsForDay = (date: Date) => {
    return events.filter(ev => isSameDay(new Date(ev.startTime), date));
  };

  // Handle day click
  const handleDayClick = (date: Date) => {
    setSelectedDate(date);
    setModalDayEvents(getEventsForDay(date));
    setModalOpen(true);
    setSignUpError(null);
  };
  ...
  return (
    <div className="min-h-screen bg-softpink-100 flex flex-col md:flex-row items-start justify-center gap-8 p-8">
      {/* Calendar grid */}
      <div className="bg-white rounded-3xl shadow-glass p-6 w-full max-w-7xl">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-3xl font-extrabold text-brand-700">Calendário</h1>
          <div className="flex gap-2">
            <button onClick={() => setSelectedDate(addDays(selectedDate, -30))} className="px-3 py-1 rounded bg-brand-100 text-brand-700">&lt;</button>
            <span className="font-bold text-lg">{format(monthStart, 'MMMM yyyy', { locale: pt })}</span>
            <button onClick={() => setSelectedDate(addDays(selectedDate, 30))} className="px-3 py-1 rounded bg-brand-100 text-brand-700">&gt;</button>
          </div>
        </div>
        <div className="grid grid-cols-7 gap-4 mb-2">
          {WEEK_DAYS.map((d, i) => (
            <div key={i} className="text-center font-bold text-brand-700">{d}</div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-4">
          {days.map((date, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`rounded-xl min-h-[160px] p-3 cursor-pointer border transition-all flex flex-col gap-2 ${isSameMonth(date, monthStart) ? 'bg-softpink-50 hover:bg-softpink-100 border-brand-100' : 'bg-gray-50 text-gray-400 border-gray-100'} ${isSameDay(date, today) ? 'ring-2 ring-brand-500' : ''}`}
                onClick={() => handleDayClick(date)}
                style={{ minWidth: '140px', maxWidth: '180px' }}
              >
                <div className="text-base font-bold text-brand-700 mb-2 text-right">{format(date, 'd')}</div>
                {dayEvents.map(ev => {
                  const signUps = signUpsMap[ev.id] || [];
                  ...
                  return (
                    <div key={ev.id} className={`rounded-lg px-2 py-1 flex flex-col gap-1 ${eventColor} shadow-sm`}>
                      <div className="flex items-center gap-2">
                        <span className="text-lg">
                          {ev.type === 'Turno' ? '☎️' : ev.type === 'Teambuilding' ? '🎉' : ev.type === 'Evento Aberto' ? '📢' : '💻'}
                        </span>
                        <span className="font-semibold text-xs truncate max-w-[90px]" title={ev.title}>{ev.title}</span>
                        <span className={`ml-1 w-3 h-3 rounded-full inline-block border border-white shadow ${statusBall}`}></span>
                      </div>
      ...
      <div className="bg-white rounded-3xl shadow-glass p-6 w-full max-w-xs min-w-[320px]">
        <h2 className="text-xl font-extrabold text-brand-700 mb-4 flex items-center gap-2">Esta semana <span className="text-2xl">📅</span></h2>
      ...
      {modalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50">
          <div className="bg-white rounded-2xl shadow-lg border-2 border-[#D29674] w-full max-w-lg p-8 flex flex-col items-center relative">
            <button className="absolute top-4 right-4 text-2xl text-gray-400 hover:text-gray-700" onClick={() => setModalOpen(false)}>&times;</button>
            <h2 className="text-xl font-bold text-brand-700 mb-4">Eventos em {format(selectedDate, 'd MMMM yyyy', { locale: pt })}</h2>
            {modalDayEvents.length === 0 ? (
              <div className="text-brand-400">Nenhum evento neste dia.</div>
            ) : (
              <ul className="w-full flex flex-col gap-4 max-h-[60vh] overflow-y-auto p-2 -mr-2 pr-4">
                {modalDayEvents.map(ev => {
                  ...
                  return (
                    <li key={ev.id} className={`rounded-xl border p-4 flex flex-col gap-2 ${ev.status === 'published' ? 'border-brand-200' : 'border-gray-200 bg-gray-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xl">
                            {ev.type === 'Turno' ? '☎️' : ev.type === 'Teambuilding' ? '🎉' : ev.type === 'Evento Aberto' ? '📢' : '💻'}
                          </span>
                          <span className="font-bold text-lg text-brand-700">{ev.title}</span>
                      ...
                      {ev.description && (
                        <div className="mt-2 border-t border-brand-100 pt-2">
                          <p className="text-sm text-brand-800 max-h-24 overflow-y-auto">{ev.description}</p>
                        </div>
                      )}
                      ...
                      <div className="flex flex-col gap-2 mt-2">
                        ...
                        {user && ev.status === 'published' && (
                          isUserSignedUp ? (
                            <button
                              className="w-full px-4 py-2 rounded-lg bg-gray-200 text-brand-700 font-bold shadow hover:bg-gray-300 transition text-base mt-2"
                            ...
                            <button
                              className="w-full px-4 py-2 rounded-lg bg-[#D29674] text-white font-bold shadow hover:bg-[#b97b54] transition text-base mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
